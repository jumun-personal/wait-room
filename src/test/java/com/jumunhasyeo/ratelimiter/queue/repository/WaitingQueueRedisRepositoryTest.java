...
class WaitingQueueRedisRepositoryTest {
    ...
    class EnterTests {
        ...
        void 토큰이_남아있으면_즉시_ACTIVE_상태가_된다() {
            String result = repository.enterOrQueue("user-1", "token-1", 10, 600);
        ...
        void 토큰이_모두_소진되면_대기열에_추가된다() {
            // 토큰 1개만 설정하고 1명 입장
            repository.enterOrQueue("user-1", "token-1", 1, 600);

            // 2번째 사용자는 대기열에 들어감
            String result = repository.enterOrQueue("user-2", "token-2", 1, 600);
        ...
        void 이미_대기열에_있는_사용자는_중복_등록되지_않는다() {
            repository.enterOrQueue("user-1", "token-1", 1, 600); // 입장
            repository.enterOrQueue("user-2", "token-2", 1, 600); // 대기열
            String result = repository.enterOrQueue("user-2", "token-3", 1, 600); // 중복 시도
        ...
        void 이미_활성_상태인_사용자는_ALREADY_ACTIVE를_반환한다() {
            repository.enterOrQueue("user-1", "token-1", 10, 600);
            String result = repository.enterOrQueue("user-1", "token-2", 10, 600);
        ...
        void FIFO_순서로_대기열_순위가_매겨진다() throws InterruptedException {
            repository.enterOrQueue("active-user", "t-0", 1, 600);

            String rank1 = repository.enterOrQueue("user-1", "t-1", 1, 600);
            Thread.sleep(10); // score 차이를 위해
            String rank2 = repository.enterOrQueue("user-2", "t-2", 1, 600);
    ...
    class PollTests {
        ...
        void 순번이_도달하면_ADMITTED_상태가_된다() {
            // 토큰 2개, 1명 입장 → 1자리 남음
            repository.enterOrQueue("user-1", "token-1", 2, 600);
            repository.enterOrQueue("user-2", "queue-token", 1, 600); // 대기열 진입 (maxTokens=1)

            // user-1 제거하여 자리 확보 (active 2칸 중 user-1만 있으니 user-2 입장 가능)
            redisTemplate.opsForZSet().remove(QueueRedisKeys.ACTIVE_SET, "user-1");

            String result = repository.poll("user-2", "queue-token", "active-token", 1, 600);
        ...
        void 순번이_아직_아니면_현재_순위를_반환한다() {
            repository.enterOrQueue("active-user", "t-0", 1, 600);
            repository.enterOrQueue("user-1", "token-1", 1, 600); // rank 0

            String result = repository.poll("user-1", "token-1", "at", 1, 600);
            assertThat(result).isEqualTo("0");
        }
        ...
        void 잘못된_토큰이면_INVALID_TOKEN을_반환한다() {
            repository.enterOrQueue("active-user", "t-0", 1, 600);
            repository.enterOrQueue("user-1", "real-token", 1, 600);

            String result = repository.poll("user-1", "wrong-token", "at", 1, 600);
        ...
        void 대기열에_없는_사용자가_폴링하면_NOT_IN_QUEUE를_반환한다() {
            ...
            String result = repository.poll("ghost", "t-1", "at", 10, 600);
    ...
    class CleanupTests {
        ...
        void 만료된_활성_사용자가_제거된다() throws InterruptedException {
            repository.enterOrQueue("user-1", "t-1", 10, 600);
        ...
        void stale_폴링_사용자가_대기열에서_제거된다() {
            repository.enterOrQueue("active-user", "t-0", 1, 600);
            repository.enterOrQueue("user-1", "t-1", 1, 600);
